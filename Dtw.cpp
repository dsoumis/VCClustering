...
template<>
double
calculateC(vector<vector<Cell>> &C, vector<pair<double, double>> const &p, vector<pair<double, double>> const &q) {
    double distance;
    for (unsigned long i = 0; i < p.size(); i++) {
        for (unsigned long j = 0; j < q.size(); j++) {
            ...
            if (i > 0 && j > 0) {
                ...
                C[i][j].i = (int) i;                      //keep the place in the array
                C[i][j].j = (int) j;
...
template<>
double Dtw(vector<pair<double, double>> const &p, vector<pair<double, double>> const &q, vector<vector<Cell>> &C) {

    C.resize(p.size());
    for (unsigned long i = 0; i < p.size(); i++) {
        C[i].resize(q.size());
    }
    C[0][0].value = calculateDistance(p[0].first, p[0].second, q[0].first,
                                      q[0].second);              //initialize first value with ||p-q||
